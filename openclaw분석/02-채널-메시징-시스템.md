...
모든 채팅 채널은 `extensions/` 아래의 번들 채널 확장 패키지로 구현되며(현재 약 22개), 각 채널은 수신, 메시지 정규화, 라우팅, 응답 전송의 일관된 파이프라인을 따른다.

- **번들 채널 확장 (22개)**: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `qqbot`, `signal`, `slack`, `synology-chat`, `telegram`, `tlon`, `twitch`, `whatsapp`, `zalo`, `zalouser`
...
flowchart TB
    ...
    subgraph ChannelsPrimary["주요 채팅 채널 (extensions/)"]
        ...
        WA["WhatsApp<br/>Baileys 7.0.0-rc.9"]
        DC["Discord<br/>discord-api-types ^0.38.45"]
        GC["Google Chat<br/>Chat API"]
        SL["Slack<br/>@slack/bolt ^4.7"]
        SG["Signal<br/>signal-utils 0.21.1"]
...
| 채널 | 라이브러리 | 소스 위치 | 프로토콜 |
|------|-----------|-----------|----------|
| **WhatsApp** | `@whiskeysockets/baileys` 7.0.0-rc.9 | `extensions/whatsapp/` | WhatsApp Web (QR 링크) |
| **Telegram** | `grammy` ^1.42.0 | `extensions/telegram/` | Telegram Bot API |
| **Discord** | `discord-api-types` ^0.38.45 | `extensions/discord/` | Discord Bot API |
| **Google Chat** | Chat API (HTTP webhook) | `extensions/googlechat/` | Google Workspace Chat |
| **Slack** | `@slack/bolt` ^4.7.0 | `extensions/slack/` | Slack Socket Mode |
| **Signal** | `signal-utils` 0.21.1 | `extensions/signal/` | signal-cli REST API |
| **iMessage** | imsg (macOS 네이티브) | `extensions/imessage/` | macOS Messages 프레임워크 |
...
| **Signal** | `extensions/signal/` | Signal (signal-cli) |
| **iMessage** | `extensions/imessage/` | macOS 네이티브 iMessage |
| **BlueBubbles** | `extensions/bluebubbles/` | BlueBubbles 서버 경유 iMessage |
...
이외에도 `extensions/` 에는 채널이 아닌 모델 프로바이더·음성/영상·메모리 등 다양한 플러그인이 함께 번들된다(예: `voice-call`, `copilot-proxy`, `memory-lancedb`, `diagnostics-otel` 등).
