...
| 항목 | 상세 |
|------|------|
| 워크스페이스 패키지 | `packages/plugin-sdk`, `packages/plugin-package-contract`, `packages/memory-host-sdk` (3개) |
...
| 번들 확장(extensions) | 113개 디렉토리/엔트리 (`extensions/`) |
...
graph TB
    subgraph PluginSystem["플러그인 시스템"]
        ...
        ChannelPlugins["채널/프로바이더/하네스<br/><code>extensions/* (113)</code>"]
...
플러그인 SDK는 pnpm 워크스페이스에 3개의 내부 패키지로 구성된다.
...
확장은 `extensions/*` 디렉토리 아래에 **워크스페이스 패키지**로 관리된다. 현재 저장소에는 113개의 엔트리(디렉토리 + 패키지 파일)가 있으며, 채널 플러그인뿐 아니라 프로바이더 플러그인(`anthropic`, `openai`, `google`, `lmstudio`, `ollama` 등)·미디어/QA 하네스(`qa-lab`, `qa-channel`, `qa-matrix`)·도구 플러그인(`browser`, `openshell`, `phone-control`)도 포함된다. 각 확장은 독립적인 `package.json`을 가지며, pnpm 워크스페이스를 통해 루트 프로젝트와 연결된다.
...
| 카테고리 | 스킬 예시 | 설명 |
|----------|----------|------|
| 개발 도구 | GitHub, GitLab, Linear | 이슈 관리, PR 생성, 코드 리뷰 |
| 생산성 | Notion, Google Docs, Calendar | 문서 작성, 일정 관리 |
| 커뮤니케이션 | Slack, Discord, Email | 메시지 전송, 알림 관리 |
| 미디어 | Spotify, YouTube | 음악 재생, 영상 검색 |
| 브라우저 | Browser Automation | 웹 페이지 조작, 스크린샷 |
| UI | Canvas | A2UI 기반 인터랙티브 캔버스 |
| 스케줄링 | Cron | 예약 작업 실행 |
| 데이터 | Database, API | 데이터 조회, API 호출 |
...
```
extensions/                         # 번들 확장 (채널/프로바이더/도구/메모리/QA), 113 엔트리
├── slack/ discord/ telegram/ matrix/ imessage/ msteams/ line/ ...
├── anthropic/ openai/ google/ ollama/ lmstudio/ groq/ moonshot/ ...
├── browser/ openshell/ phone-control/ diffs/ image-generation-core/ ...
├── memory-lancedb/ memory-wiki/ memory-core/ ...
└── qa-lab/ qa-channel/ qa-matrix/ diagnostics-otel/ ...
```
