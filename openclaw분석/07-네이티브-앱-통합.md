...
`apps/ios/version.json`이 기준 버전이며, 빌드 시 `Info.plist`의 `CFBundleShortVersionString`과 동기화된다.
...
2. **Bonjour 탐색**: `_openclaw-gw._tcp` mDNS 서비스로 로컬 네트워크에서 게이트웨이를 자동 검색
...
│   ├── version.json                    # 버전 소스 (CFBundle* 동기화 대상)
│   ├── Config/                         # 빌드 설정
...
│   ├── app/
│   │   ├── build.gradle.kts            # 앱 빌드 설정 (버전 포함)
