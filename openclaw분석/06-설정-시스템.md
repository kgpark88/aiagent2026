...
| 마이그레이션 | `legacy.ts`, `legacy.rules.ts`, `legacy.shared.ts` | 이전 버전(clawdbot) 설정 자동 변환 |
...
이전 버전(리브랜딩 전 `clawdbot`)의 설정 포맷을 현재 스키마에 맞게 자동 변환한다. 구현은 `src/config/legacy.ts`, `legacy.rules.ts`, `legacy.shared.ts`에 분산되어 있으며, `doctor` 명령이 실행 시점에 `applyRuntimeLegacyConfigMigrations()`를 호출해 런타임 호환 계층도 적용한다. 사용자가 버전 업그레이드 시 설정을 수동으로 수정할 필요가 없도록 보장한다.

```mermaid
flowchart TD
    A["설정 로드 시도"] --> B{"레거시 포맷<br/>감지?"}
    B -->|"예"| C["migrateLegacyConfig()"]
    C --> D["필드명 변환"]
    D --> E["구조 재배치"]
    E --> F["기본값 삽입"]
    F --> G["마이그레이션된 설정 저장"]
...
├── legacy.ts / legacy.rules.ts / legacy.shared.ts  # 레거시 마이그레이션
