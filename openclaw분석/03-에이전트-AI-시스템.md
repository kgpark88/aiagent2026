...
OpenClaw의 에이전트 시스템은 외부 `pi-*` 패키지에 의존하던 과거와 달리, 이제 **내장(built-in) 에이전트 런타임**을 직접 소유합니다. 런타임 코어는 `packages/agent-core`(`@openclaw/agent-core`) 와 `src/agents/embedded-agent-runner/` 에 있으며, 모델·프로바이더 헬퍼는 `src/llm/`·`packages/llm-core`·`packages/llm-runtime` 로 분리되었습니다. 퍼스낼리티 계층은 여전히 `SOUL.md` 로 표현되는 "soul" 개념입니다. RPC 기반 명령 실행, 도구 스트리밍, 멀티 프로바이더 모델 라우팅을 통해 다양한 채널에서 수신된 사용자 메시지를 처리하고 AI 응답을 생성합니다.

### 핵심 특성

| 특성 | 설명 |
|------|------|
| 런타임 엔진 | 임베디드 에이전트 러너(`runEmbeddedAgent`) + 이벤트 구독(`subscribeEmbeddedAgentSession`) |
...
| 모델 라우팅 | 수십 개의 모델 프로바이더(`extensions/` 프로바이더 플러그인으로 동적 등록), 페일오버, 프로필 로테이션 |
...
flowchart TB
    subgraph AgentRuntime["에이전트 런타임"]
        Embedded["Embedded Agent Runner"]
...
에이전트/LLM 코어는 더 이상 외부 `@mariozechner/pi-*` 패키지가 아니라, OpenClaw 리포지토리 내부의 워크스페이스 패키지로 이관되었습니다.

| 패키지 | 버전 | 역할 |
|--------|------|------|
| `@openclaw/agent-core` | 워크스페이스 (`packages/agent-core`) | 에이전트 코어 (메시지 타입, 스트림, 도구/세션 계약, 컴팩션 헬퍼) |
| `@openclaw/llm-core` | 워크스페이스 (`packages/llm-core`) | LLM 모델 추상화 (프로바이더, 스트리밍, 컨텍스트) |
| `@openclaw/llm-runtime` | 워크스페이스 (`packages/llm-runtime`) | LLM 런타임 (전송, 스트림 어댑터) |
| `@agentclientprotocol/sdk` | 0.22.1 | ACP (Agent Client Protocol) SDK — `src/acp/` 에서 Gateway 세션과 브리지 |
| `typebox` | 1.1.39 | JSON Schema 기반 도구 파라미터 정의 |
| `@earendil-works/pi-tui` | 0.78.0 | 터미널 UI 컴포넌트 (3rd-party, TUI/세션 렌더러 전용) |

> OpenClaw 2026.6.11 기준. `engines.node >= 22.19.0`, `packageManager: pnpm@11.2.2`.

```typescript
// 핵심 임포트 패턴
import type { AgentMessage, StreamFn, AgentTool, AgentToolResult } from "@openclaw/agent-core";
...
export type ResolvedCliBackend = {
  id: string;
  modelProvider?: string;                  // 백엔드가 소유하는 모델 프로바이더
  ...
  // 이 밖에 defaultAuthProfileId, authEpochMode, nativeToolMode 등 다수 필드 존재
};
```

모델 별칭은 각 백엔드의 모델 별칭 설정에 정의되며, 최신 Claude 모델(`claude-opus-4-5`, `claude-sonnet-4-5`, `claude-haiku-4-5` 등) 및 단축 별칭(`opus`, `sonnet`, `haiku`) 을 정규화합니다. 정확한 버전 목록은 릴리스마다 갱신되므로 `extensions/anthropic/` 등의 프로바이더 플러그인을 기준으로 확인하는 것이 안전합니다.
...
```typescript
export const CONTEXT_WINDOW_HARD_MIN_TOKENS = 4_000;   // 절대 최소
const CONTEXT_WINDOW_WARN_BELOW_TOKENS = 8_000;        // 경고 임계값 (내부 상수)
...
**파일:** `src/skills/`(로딩·검색·런타임·보안 등 서브모듈), 세션 배선은 `src/agents/sessions/`

스킬 시스템은 과거 `src/agents/skills.ts` 에서 최상위 `src/skills/` 모듈로 이관되었습니다. 50여 개의 번들 스킬을 관리하며, 에이전트별로 허용 스킬을 필터링하고 온디맨드로 등록합니다.

### 번들 스킬 (`skills/` 디렉토리, 약 50여 개)
...
gifgrep, github, gog, goplaces, healthcheck, himalaya, imsg, mcporter,
meme-maker, model-usage, nano-pdf, node-connect, node-inspect-debugger,
notion, obsidian, openai-whisper, openai-whisper-api, openhue, oracle,
ordercli, peekaboo, python-debugpy, sag, session-logs, sherpa-onnx-tts,
skill-creator, songsee, sonoscli, spike, spotify-player, summarize,
taskflow, taskflow-inbox-triage, things-mac, tmux, trello, video-frames,
weather, xurl
```

> 번들 스킬 목록은 릴리스마다 추가/제거되므로 정확한 집합은 배포본의 `skills/` 디렉토리를 기준으로 확인합니다.
...
```typescript
// 스킬 내보내기 (src/skills/loading/workspace.ts 등)
export {
...
} from "src/skills/loading/workspace.js";
...
이와 동일한 방식으로 수십 개의 모델 프로바이더(anthropic, openai, google, xai, openrouter, groq, mistral, perplexity, deepseek, moonshot, qwen, zai, ollama, lmstudio, vllm, fireworks, together, venice, minimax, nvidia, vercel-ai-gateway 등 다수)가 `extensions/` 에 플러그인으로 제공됩니다.
...
export class FailoverError extends Error {
  // FailoverReason: auth | auth_permanent | format | rate_limit | overloaded
  //               | billing | timeout | context_overflow | model_not_found | server_error ...
...
// 에러 분류 매핑 (resolveFailoverStatus)
// 402 -> billing, 429 -> rate_limit, 401 -> auth, 403 -> auth_permanent
// 408 -> timeout, 413 -> context_overflow, 404 -> model_not_found
// 500 -> server_error, 503 -> overloaded, 400 -> format
...
**파일:** `src/mcp/`, `src/agents/mcp-*.ts`, `src/agents/agent-bundle-mcp-*.ts`
...
- `src/agents/agent-bundle-mcp-runtime.ts`, `agent-bundle-mcp-tools.ts` — CLI 백엔드에 MCP 툴 번들 주입 (과거 `pi-bundle-mcp-*`)
...
│
├── embedded-agent.ts                 # 임베디드 에이전트 모드 진입점
├── embedded-agent-runner/            # 임베디드 에이전트 러너 (run.ts 등)
├── embedded-agent-runner.ts          # 러너 재-내보내기 (runEmbeddedAgent)
├── embedded-agent-subscribe.ts       # 임베디드 에이전트 구독(subscribeEmbeddedAgentSession)
├── embedded-agent-helpers.ts         # 임베디드 에이전트 헬퍼
├── embedded-agent-messaging.ts       # 임베디드 에이전트 메시징
├── embedded-agent-utils.ts           # 임베디드 에이전트 유틸리티
├── embedded-agent-block-chunker.ts   # 블록 청킹
│
├── agent-tools.ts                    # 에이전트 도구 정의 (과거 pi-tools.ts)
├── agent-tools.params.ts             # 도구 파라미터/스키마
├── agent-tools.types.ts              # 도구 타입
├── agent-tools.policy.ts             # 도구 정책
├── agent-tools.abort.ts              # 도구 중단
├── agent-tools.before-tool-call.ts   # 도구 호출 전 훅
├── agent-tools.read.ts               # 읽기 도구
├── agent-tools-parameter-schema.ts   # 도구 정의 파라미터 스키마
│
...
│
│   # 스킬 시스템은 최상위 src/skills/ 로 이관됨
│   #   (loading, discovery, runtime, security, config, workshop 등 서브모듈)
│   # 세션 배선은 src/agents/sessions/ 에서 처리
│
...
├── agent-bundle-mcp-runtime.ts       # CLI 백엔드용 MCP 번들 런타임 (과거 pi-bundle-mcp-*)
├── agent-bundle-mcp-tools.ts         # 번들된 MCP 도구 주입
│
├── usage.ts                          # 사용량 추적
...
1. **임베디드 에이전트 러너** - 핵심 에이전트 런타임으로, 모델 API 호출과 도구 실행 루프를 관리 (`runEmbeddedAgent` + `subscribeEmbeddedAgentSession`)
...
10. **모델 프로바이더** - 수십 개의 프로바이더(`extensions/`) 플러그인으로 동적 등록 및 카탈로그 주입
