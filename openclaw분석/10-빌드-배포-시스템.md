...
openclaw의 빌드 & 배포 시스템은 **pnpm 11 워크스페이스 모노레포**를 기반으로 구성되어 있다. `tsdown` 번들러로 TypeScript ESM 소스를 컴파일하고, 단일 멀티스테이지 `Dockerfile`(빌드 스테이지는 bookworm, 런타임 스테이지는 bookworm-slim)로 컨테이너 이미지를 만든다. 코드 실행 샌드박스 격리는 별도 Dockerfile이 아니라 런타임 설정(`agents.defaults.sandbox`)과 Docker CLI 마운트로 제공되며, Fly.io·Render·Kubernetes 등 다양한 배포 타깃을 지원하고, GitHub Actions를 활용한 CI/CD 파이프라인으로 자동화된다.

| 항목 | 기술 스택 |
|------|-----------|
| 패키지 관리 | pnpm 11 워크스페이스 (`packageManager: pnpm@11.2.2`) |
| Node 요구사항 | `>=22.19.0` |
...
| 컨테이너 | Docker (빌드 `node:24-bookworm` / 런타임 `node:24-bookworm-slim`) |
...
plugins:assets:build           # 번들 플러그인 에셋 빌드 (bundled-plugin-assets.mjs)
plugins:assets:copy            # 번들 플러그인 에셋 복사
...
graph TB
    ...
    Packages["packages/*<br/>공유 패키지 (21개)"]
    Extensions["extensions/*<br/>확장 플러그인 (145개)"]
...
| 플랫폼 | `platform: "node"` |
| 출력 형식 | ESM |
| 타겟 | Node.js 22+ (`engines: >=22.19.0`) |
| 타입 선언 | `build:plugin-sdk:dts`(tsgo)로 별도 생성 |
...
단일 멀티스테이지 `Dockerfile`이 빌드부터 런타임까지를 담당한다. 별도의 `Dockerfile.sandbox*`는 더 이상 존재하지 않으며, 코드 실행 샌드박스는 런타임 설정(`agents.defaults.sandbox`)과 Docker CLI 마운트로 활성화한다.

```mermaid
graph TB
    subgraph DockerStages["Dockerfile 멀티스테이지"]
        WSDeps["workspace-deps<br/>node:24-bookworm<br/>package.json 매니페스트 추출"]
        Build["build<br/>node:24-bookworm<br/>Bun + pnpm build:docker"]
        RtAssets["runtime-assets<br/>dist/node_modules staging"]
        Runtime["runtime<br/>node:24-bookworm-slim<br/>최종 런타임 이미지"]
    end

    subgraph RuntimeCfg["런타임 설정"]
    ...
    style DockerStages fill:#1a1a2e,stroke:#e94560,color:#eee
    style RuntimeCfg fill:#16213e,stroke:#0f3460,color:#eee
```
...
ARG OPENCLAW_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:..."
ARG OPENCLAW_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:..."
ARG OPENCLAW_BUN_IMAGE="docker.io/oven/bun:1.3.13@sha256:..."
ARG OPENCLAW_EXTENSIONS=""        # 옵트인할 번들 확장 (콤마 구분)
ARG OPENCLAW_BUNDLED_PLUGIN_DIR=extensions

FROM ${OPENCLAW_NODE_BOOKWORM_IMAGE} AS workspace-deps
# packages/*, extensions/*의 package.json 만 추출하여 install 레이어 캐시 보존

FROM ${OPENCLAW_BUN_IMAGE} AS bun-binary
FROM ${OPENCLAW_NODE_BOOKWORM_IMAGE} AS build
# Bun 바이너리 + pnpm 설치 후 pnpm build:docker 실행

FROM build AS runtime-assets
# dist/ 및 런타임 node_modules staging
...
| Docker 설정 | 값 |
|-------------|-----|
| 빌드 베이스 이미지 | `node:24-bookworm` (SHA256 pinned) + `oven/bun` |
| 런타임 베이스 이미지 | `node:24-bookworm-slim` (SHA256 pinned) |
| 멀티스테이지 | `workspace-deps` → `build` → `runtime-assets` → `runtime` |
| 옵트인 확장 | `--build-arg OPENCLAW_EXTENSIONS="diagnostics-otel,matrix"` |
| 엔트리포인트 | `tini -s --` (시그널 처리) |
| 기본 CMD | `node openclaw.mjs gateway` |
...
저장소 루트에는 단일 `Dockerfile`만 존재한다. 과거의 `Dockerfile.sandbox`, `Dockerfile.sandbox-common`, `Dockerfile.sandbox-browser`는 제거되었고, 샌드박스 격리는 이미지가 아니라 런타임 설정으로 처리된다.

| 항목 | 설명 |
|------|------|
| `Dockerfile` | 메인 게이트웨이 + CLI 서비스 (멀티스테이지, 런타임은 bookworm-slim) |
| 샌드박스 격리 | `agents.defaults.sandbox` 설정 + Docker CLI/소켓 마운트로 활성화 |
| 샌드박스 셋업 | `scripts/docker/setup.sh`(`OPENCLAW_SANDBOX=1`), `scripts/docker/sandbox/` |

셋업/실행 헬퍼는 `scripts/docker/`(예: `setup.sh`, `install-sh-*`)와 `scripts/podman/`(`setup.sh`, `openclaw.container.in`), `scripts/run-openclaw-podman.sh`에 정리되어 있으며, Fly 전용 프라이빗 설정은 `deploy/fly.private.toml`에 위치한다.
...
```
stable: v2026.6.11        → npm install openclaw@latest
beta:   v2026.7.1-beta.1   → npm install openclaw@beta
...
| 검증 단계 | 도구 | 실패 시 |
|----------|------|--------|
| 린팅 | Oxlint 1.67.0 | PR 머지 차단 |
| 포맷팅 | Oxfmt 0.52.0 | PR 머지 차단 |
| 테스트 | Vitest 4.1.8 | PR 머지 차단 |
...
```
/
├── Dockerfile                     # 메인 Docker 이미지 (단일 멀티스테이지)
...
│
├── scripts/
│   ├── docker/                    # Docker 셋업/샌드박스/설치 스모크 스크립트
│   │   ├── setup.sh               # (OPENCLAW_SANDBOX=1) 샌드박스 셋업
│   │   └── sandbox/
│   ├── podman/                    # Podman 셋업 (setup.sh, openclaw.container.in)
...
└── .github/workflows/             # GitHub Actions CI/CD
    ...
    ├── windows-node-release.yml   # Windows Node 릴리스
...
> **참고**: openclaw의 빌드 & 배포 시스템은 최신 Rust 기반 도구체인(tsdown/rolldown, Oxlint, Oxfmt)을 적극 활용하여 빌드 속도와 개발자 경험을 극대화한다. 단일 Dockerfile 기반 멀티스테이지 빌드(런타임 bookworm-slim)와 설정 기반 샌드박스 격리, 그리고 Fly.io/Render/Kubernetes/Raspberry Pi 등 광범위한 배포 타깃을 함께 지원하여 단일 벤더에 종속되지 않는 배포 전략을 제공한다.
