...
Node 22 LTS 이상 필수. 보안 패치 및 `crypto.timingSafeEqual` 최신 구현을 보장하기 위함이다. `package.json`의 `engines` 필드와 Dockerfile 모두 Node 22를 지정한다.
...
| 최소 베이스 이미지 | `node:22-bookworm` (불필요한 패키지 미포함) |
