...
- **프로토콜**: WebSocket JSON-RPC (PROTOCOL_VERSION = 4)
- **HTTP 프레임워크**: Express 기반 HTTP 서버 + `ws` 기반 WebSocket (WS + HTTP 멀티플렉스)
- **버전**: `2026.6.11` (`engines.node >= 22.19.0`; 공식 문서 권장 Node 24 또는 Node 22 LTS 22.19+)
...
`src/gateway/client.ts`는 게이트웨이에 접속하는 WebSocket 클라이언트의 진입점이다.
현재 핵심 로직은 공용 패키지 `packages/gateway-client/src/client.ts`로 분리되었고,
`src/gateway/client.ts`는 이를 감싸 OpenClaw 호스트 의존성(디바이스 인증, 프록시, TLS 지문 등)을 주입하는 파사드(facade) 역할을 한다.
...
프로토콜 정의는 공용 패키지 `packages/gateway-protocol/src/`로 분리되었으며, JSON 기반 WebSocket RPC 프레임을 사용한다.
(스키마는 `packages/gateway-protocol/src/schema/`, 프레임 정의는 `schema/frames.ts`에 위치한다.)
...
// (packages/gateway-protocol/src/schema/protocol-schemas.ts 및 index.ts에서 re-export)
```
...
> 프로토콜 정의는 더 이상 `src/gateway/protocol/` 아래에 있지 않고 공용 패키지
> `packages/gateway-protocol/src/`로 분리되었으며, 클라이언트 코어는 `packages/gateway-client/src/`로 분리되었다.
> 아래 트리의 `client.ts`는 해당 패키지를 감싸는 파사드다.

```
packages/gateway-protocol/src/          # (분리됨) 프로토콜 스키마·타입 패키지
├── index.ts                            # 검증기 + 타입 re-export
├── version.ts                          # PROTOCOL_VERSION(=4) 등 버전 상수
├── schema.ts                           # 전체 프로토콜 스키마 re-export
├── schema/                             # 개별 스키마 정의
│   ├── protocol-schemas.ts             # 프레임 스키마, 버전 re-export
│   └── frames.ts                       # RequestFrame/ResponseFrame/EventFrame 스키마
├── client-info.ts                      # 클라이언트 정보 유틸
└── index.test.ts

packages/gateway-client/src/            # (분리됨) 게이트웨이 클라이언트 코어 패키지
├── client.ts                           # GatewayClient 코어 구현 + GatewayClientOptions
├── device-auth.ts                      # 디바이스 인증
└── readiness.ts / timeouts.ts          # 준비/타임아웃 유틸
...
│   └── types.ts                          # 공유 타입
├── client.ts                             # 게이트웨이 클라이언트 파사드 (gateway-client 패키지 래핑)
