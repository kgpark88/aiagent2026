...
**파일:** `packages/memory-host-sdk/src/host/memory-schema.ts`
...
**파일:** `packages/memory-host-sdk/src/host/embeddings.ts` (로컬), `embeddings-remote-provider.ts` (원격)
...
**파일:** `packages/memory-host-sdk/src/host/embedding-provider-adapter-utils.ts` (키 계산), `headers-fingerprint`는 원격 클라이언트 모듈에 내장
...
**파일:** `packages/memory-host-sdk/src/host/sqlite.ts`
...
**파일:** `packages/memory-host-sdk/src/host/sqlite-vec.ts`
...
**파일:** `packages/memory-host-sdk/src/host/types.ts` (`src/memory-host-sdk/host/types.ts`에도 노출)
...
메모리 시스템은 재사용 가능한 Host SDK와 엔진/플러그인 확장으로 분리되어 있습니다. 기본 `memory` 디렉토리는 없으며, 코어 SDK 구현체(DB/임베딩 호스트)는 워크스페이스 패키지 `packages/memory-host-sdk/src/`에 위치합니다. `src/memory-host-sdk/`에는 얇은 셋(`host/types.ts`, `host/backend-config.ts`와 `dreaming.ts`, `engine-qmd.ts`, `engine-storage.ts`, `events.ts`, `multimodal.ts`, `query.ts`, `secret.ts`, `status.ts`)만 노출됩니다. 엔진/플러그인은 `extensions/memory-*`에 위치합니다.
...
| **QMD** | `packages/memory-host-sdk/src/host/qmd-process.ts` 외 | QMD 사이드카 | QMD 검색 | 로컬 퍼스트 외부 인덱서. 쿼리 확장, 리랭킹, 워크스페이스 외부 경로 인덱싱 지원. `mcporter` 연동 가능 |
...
| `allowedChatTypes` | `direct` / `group` / `channel` / `explicit` 중 허용 범위 |
